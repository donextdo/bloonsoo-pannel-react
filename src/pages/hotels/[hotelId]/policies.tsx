...
const PoliciesPage = () => {
    ...
    const [accommodateChildrenError, setAccommodateChildrenError] = useState('yes');
    ...
    const getCurrentHotel = async () => {
        try {
            ...
            setCancellationDuration(data.policies.cancellation_duration)
            setPayTime(data.policies.pay_time)
            setPreventAccidentalBookings(data.policies.preventAccidental_bookings)
            setCheckInForm(data.policies.check_in_form)
            setCheckInUntill(data.policies.check_in_untill)
            setCheckOutForm(data.policies.check_out_form)
            setCheckOutUntill(data.policies.check_out_untill)
            setAccommodateChildren(data.policies.accommodate_children ? 'yes' : 'no')
            setPets(data.policies.pets ? 'yes' : 'no')
    ...
    const handleSave = async () => {
        ...
        const dto = {
            ...
            accommodate_children: accommodateChildren === 'yes' ? true : false,
    ...
    return (
        <Content>
            <RouteNavigationBar previous={"Images"} current={"Policies"} next={"Payment"} handleEditToggle={handleEditToggle} editMode={editMode} handleNext={handleNext} handlePrevious={handlePrevious} handleSave={handleSave}/>

            <section
                className=" flex flex-col gap-8 text-black font-montserrat mt-[26px]"
            >
