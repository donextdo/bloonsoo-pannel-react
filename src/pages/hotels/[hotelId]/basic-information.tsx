...
const BasicInformation = () => {
    const [hotelIid, setHotelIid] = useState();
    ...
    const getCurrentHotel = async () => {
        try {
            ...
            setStartRating(data.star_rating)
            setContactName(data.contact_name)
            setPhoneNumber(data.contact_phone_number)
            setPhoneNumberAlt(data.contact_phone_number_alternative)
            setOwnMultipleHotels(data.is_own_multiple_hotels)
            setChannelManager(data.use_channel_manager)
            setCountry(data.property_address.country)
            setPostalCode(data.property_address.postal_code)
            setSearchAddress(data.property_address.street_address)
        } catch (error) {
            console.log(error)
        }
    }
    ...
    return (
        <Content>
            <RouteNavigationBar current={"Basic information"} next={"Facilities and Amenities"} handleEditToggle={handleEditToggle} editMode={editMode} handleSave={handleSave} handleNext={handleNext}/>
                ...
                >
                    ...
                    <FormCard label="Contact Details">
                        <div className="grid grid-cols-2 gap-x-8 gap-y-6 px-4">
                            ...
                            <div className="flex flex-col gap-2 items-start ">
                                <label
                                    className={`text-sm font-semibold ${phoneNumberError ? "text-red-600" : "text-gray-600"
                                        }`}
                                >
                                    Phone Number
                                </label>
